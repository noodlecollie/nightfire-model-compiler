...
namespace JbnLib.Qc
{
    public class QcFileV11 : QcFile
    {
        ...
        public void Clear()
        {
            ...
            _Root = null;
            _Scale = null;
            _SequenceCollection = new List<CommandSequenceV11>();
            _SoundGroupCollection = new List<CommandSoundGroup>();
            _TextureCollection = new List<CommandTexture>();
            _TextureGroup = null;
            ...
            while (!eof)
            {
                try
                {
                    ...
                    switch (_Tokenizer.Token.ToLower())
                    {
                        ...
                        #region " Include "
                        case CommandInclude.Command:
                            _Tokenizer.GetToken();
                            _Include = new CommandInclude(_Tokenizer.Token);
                            break;
                        #endregion
                        ...
                        #region " Sequence "
                        case CommandSequenceV11.Command:
                            ...
                            while (!_Tokenizer.GetToken())
                            {
                                ...
                                switch (_Tokenizer.Token.ToLower())
                                {
                                    ...
                                    default:
                                        ...
                                        ActivityV11 activitytemp;
                                        try { activitytemp = (ActivityV11)Enum.Parse(typeof(ActivityV11), _Tokenizer.Token); }
                                        catch (ArgumentException) { activitytemp = ActivityV11.ACT_INVALID; }

                                        if (controltemp != MotionFlags.Invalid)
                                            sequence.Control.Flags |= controltemp;
                                        else if (activitytemp != ActivityV11.ACT_INVALID)
                                        {
                                            OptionActivityV11 activity = new OptionActivityV11();
                        ...
                        #region " Sound Group "
                        case CommandSoundGroup.Command:
                            CommandSoundGroup soundgroup = new CommandSoundGroup();
                            _Tokenizer.GetToken();
                            soundgroup.Name = _Tokenizer.Token;
                            _Tokenizer.GetToken();
                            if (_Tokenizer.Token == "{")
                            {
                                while (_Tokenizer.Token[0] != '}')
                                {
                                    _Tokenizer.GetToken();
                                    if (_Tokenizer.Token[0] != '}')
                                        soundgroup.Sounds.Add(_Tokenizer.Token);
                                }
                            }
                            _SoundGroupCollection.Add(soundgroup);
                            break;
                        #endregion

                        #region " Texture "
                        case CommandTexture.Command:
                            CommandTexture texture = new CommandTexture();
                            _Tokenizer.GetToken();
                            texture.Texture = _Tokenizer.Token;
                            _Tokenizer.GetToken();
                            texture.Material = _Tokenizer.Token;
                            _TextureCollection.Add(texture);
                            break;
                        #endregion
                ...
                {
                    Messages.ThrowException("Qc.QcFileV11.Read(string)", e.Message + " (" + Messages.LINE + _Tokenizer.Line + ", " + Messages.TOKEN + _Tokenizer.Token + ")");
                }
            }
        }
        public void Write(string file)
        {
            ...
            if (_Include != null)
                sw.WriteLine(_Include.ToString());
            ...
            #region " Textures "
            foreach (CommandTexture texture in _TextureCollection)
                sw.WriteLine(texture.ToString());
            sw.WriteLine();
            sw.Flush();
            ...
            #region " Sound Groups "
            foreach (CommandSoundGroup soundgroup in _SoundGroupCollection)
                sw.WriteLine(soundgroup.ToString());
            sw.WriteLine();
            sw.Flush();
            #endregion
            ...
            #region " Sequences "
            foreach (CommandSequenceV11 sequence in _SequenceCollection)
            {
